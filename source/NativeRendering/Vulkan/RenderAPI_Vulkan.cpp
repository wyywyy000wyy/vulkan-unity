...
#define RAPI ((RenderAPI_Vulkan*)NM->renderApi)
#define RDEV (RAPI->m_Instance.device)
#define PDEV (RAPI->m_Instance.physicalDevice)
#define PCMDPOLL (RAPI->commandPool)
#define GQ (RAPI->m_Instance.graphicsQueue)
#define QFI (RAPI->m_Instance.queueFamilyIndex)
...
public:
    ...
    virtual void* CopyBufferData(void* bufferHandle, size_t* outBufferSize, void* target);
...
void createCommandPool() {
    ...
    if (vkCreateCommandPool(RDEV, &poolInfo, nullptr, &RAPI->commandPool) != VK_SUCCESS) {
        throw std::exception("failed to create graphics command pool!");
    }
}
...
void copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    ...
    allocInfo.commandPool = PCMDPOLL;
    allocInfo.commandBufferCount = 1;

    VkCommandBuffer commandBuffer;
    vkAllocateCommandBuffers(RDEV, &allocInfo, &commandBuffer);
    ...
    vkFreeCommandBuffers(RDEV, PCMDPOLL, 1, &commandBuffer);
}
...
void createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    if (vkCreateBuffer(RDEV, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
        throw std::exception("failed to create buffer!");
    }

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(RDEV, buffer, &memRequirements);
    ...
    if (vkAllocateMemory(RDEV, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
        throw std::exception("failed to allocate buffer memory!");
    }

    vkBindBufferMemory(RDEV, buffer, bufferMemory, 0);
}
...
void createVertexBuffer2(NativeMesh* mesh)
{
    ...
    if (!RAPI->m_UnityVulkan->AccessBuffer(mesh->vertexBuffer, 0, 0, kUnityVulkanResourceAccess_ObserveOnly, &bufferInfo))
    {
        return;
    }
...
void createIndexBuffer2(NativeMesh* mesh)
{
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    UnityVulkanBuffer bufferInfo;
    if (!RAPI->m_UnityVulkan->AccessBuffer(mesh->indexBuffer, 0, 0, kUnityVulkanResourceAccess_ObserveOnly, &bufferInfo))
    {
        return;
    }
...
{
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    VkDeviceSize bufferSize = sizeof(float)* 3 * mesh->vertexCount;
    ...
    void* data;
    vkMapMemory(RDEV, stagingBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, mesh->vertexBuffer, (size_t)bufferSize);
    vkUnmapMemory(RDEV, stagingBufferMemory);
    ...
    vkDestroyBuffer(RDEV, stagingBuffer, nullptr);
    vkFreeMemory(RDEV, stagingBufferMemory, nullptr);
}

void createIndexBuffer(NativeMesh* mesh) {
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    VkDeviceSize bufferSize = sizeof(int) * mesh->indexCount;
...
void createNativeMesh(NativeMesh* mesh)
{
    ...
    mesh->native = m;
    createVertexBuffer2(mesh);
    createIndexBuffer2(mesh);
}

void createNativeMesh2(NativeMesh* mesh)
{
    size_t bufferSize;
    void* bufferDataPtr = RAPI->BeginModifyVertexBuffer(mesh->vertexBuffer, &bufferSize);
    if (!bufferDataPtr)
        return;
    mesh->vertexBuffer = new float[mesh->vertexCount * sizeof(float) * 3];
    memcpy(mesh->vertexBuffer, bufferDataPtr, mesh->vertexCount * sizeof(float) * 3);
    mesh->vertexCount = mesh->vertexCount;

    //void* indexbufferDataPtr = RAPI->BeginModifyVertexBuffer(mesh->indexBuffer, &bufferSize);

    //mesh->indexBuffer = new int[mesh->indexCount * sizeof(int)];
    //memcpy(mesh->indexBuffer, indexbufferDataPtr, mesh->indexCount * sizeof(int));

    createNativeMesh2(mesh);
}

void RenderAPI_Vulkan::DrawMesh(NativeMesh* mesh,const float* worldMatrix)
{
    UnityVulkanRecordingState recordingState;
    if (!m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        return;
...
void* RenderAPI_Vulkan::CopyBufferData(void* bufferHandle, size_t* outBufferSize, void* target)
{
    UnityVulkanRecordingState recordingState;
    if (!RAPI->m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        return NULL;

    UnityVulkanBuffer bufferInfo;
    if (!RAPI->m_UnityVulkan->AccessBuffer(bufferHandle, 0, 0, kUnityVulkanResourceAccess_ObserveOnly, &bufferInfo))
        return NULL;
    ...
    *outBufferSize = bufferInfo.sizeInBytes;
    ...
    {
        void* data;
        vkMapMemory(RDEV, bufferInfo.memory.memory, 0, bufferInfo.sizeInBytes, 0, &data);
        memcpy(target, data, bufferInfo.sizeInBytes);
        vkUnmapMemory(RDEV, bufferInfo.memory.memory);
    }
    else
    {
        memcpy(target, bufferInfo.memory.mapped, bufferInfo.sizeInBytes);
    }
}
