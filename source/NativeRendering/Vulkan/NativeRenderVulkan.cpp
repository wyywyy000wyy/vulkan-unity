...
static VKAPI_ATTR void VKAPI_CALL Hook_vkCreateCommandPool(VkDevice device, const VkCommandPoolCreateInfo* pCreateInfo, const VkAllocationCallbacks* pAllocator, VkCommandPool* pCommandPool)
{
    if (vkCreateCommandPool(device, pCreateInfo, pAllocator, pCommandPool) != VK_SUCCESS) {
        throw std::exception("failed to create graphics command pool!");
    }
    //RAPI->commandPool = *pCommandPool;
}

RenderAPI* CreateRenderAPI_Vulkan()
{
    return new NativeRenderVulkan();
}
...
void RenderAPI_Vulkan::ProcessDeviceEvent(UnityGfxDeviceEventType type, IUnityInterfaces* interfaces)
{
    ...
    case kUnityGfxDeviceEventInitialize:
        ...
        m_UnityVulkan->InterceptVulkanAPI("vkCreateCommandPool", (PFN_vkVoidFunction)Hook_vkCreateCommandPool);
        break;
...
void* RenderAPI_Vulkan::BeginModifyVertexBuffer(void* bufferHandle, size_t* outBufferSize)
{
    ...
    *outBufferSize = bufferInfo.sizeInBytes;
    ...
    return bufferInfo.memory.mapped;
}
...
/// <param name="memoryRequirements"></param>
/// <param name="memoryPropertyFlags"></param>
/// <returns></returns>

int NativeRenderVulkan::FindMemoryTypeIndex(VkPhysicalDeviceMemoryProperties const& physicalDeviceMemoryProperties, VkMemoryRequirements const& memoryRequirements, VkMemoryPropertyFlags memoryPropertyFlags)
{
    uint32_t memoryTypeBits = memoryRequirements.memoryTypeBits;
...
VkPipelineLayout NativeRenderVulkan::CreateTrianglePipelineLayout(VkDevice device)
{
...
void NativeRenderVulkan::createNativeMesh(NativeMesh* mesh)
{
...
void NativeRenderVulkan::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    UnityVulkanRecordingState recordingState;
    ...
    vkCmdCopyBuffer(recordingState.commandBuffer, srcBuffer, dstBuffer, 1, &copyRegion);


    VkSubmitInfo submitInfo{};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
    submitInfo.commandBufferCount = 1;
    submitInfo.pCommandBuffers = &recordingState.commandBuffer;

    vkQueueSubmit(m_Instance.graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
    vkQueueWaitIdle(m_Instance.graphicsQueue);
}

void NativeRenderVulkan::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    if (vkCreateBuffer(m_Instance.device, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
        throw std::exception("failed to create buffer!");
    }

    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(m_Instance.device, buffer, &memRequirements);
    ...
    allocInfo.memoryTypeIndex = FindMemoryTypeIndex(physicalDeviceProperties, memRequirements, properties);

    if (vkAllocateMemory(m_Instance.device, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
        throw std::exception("failed to allocate buffer memory!");
    }

    vkBindBufferMemory(m_Instance.device, buffer, bufferMemory, 0);
}


void NativeRenderVulkan::createVertexBuffer(NativeMesh* mesh)
{
    //VulkanMesh* m = (VulkanMesh*)mesh->native;
    //VkDeviceSize bufferSize = sizeof(float) * 4 * mesh->vertexCount;

    //VkBuffer stagingBuffer;
    //VkDeviceMemory stagingBufferMemory;

    //CreateVulkanBuffer(bufferSize, &m->vk_vertexBuffer, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

    //memcpy(m->vk_vertexBuffer.mapped, mesh->vertexBuffer, bufferSize);
    VulkanMesh* m = (VulkanMesh*)mesh->native;

    /////////////////////
	VkDeviceSize bufferSize = 4 * 4 * mesh->vertexCount;
    ...
    void* data;
    vkMapMemory(m_Instance.device, stagingBufferMemory, 0, bufferSize, 0, &data);
    memcpy(data, mesh->vertexBuffer, (size_t)bufferSize);
    vkUnmapMemory(m_Instance.device, stagingBufferMemory);
    ...
    copyBuffer(stagingBuffer, m->vertexBuffer, bufferSize);

    vkDestroyBuffer(m_Instance.device, stagingBuffer, nullptr);
    vkFreeMemory(m_Instance.device, stagingBufferMemory, nullptr);
}

bool NativeRenderVulkan::CreateVulkanBuffer(size_t sizeInBytes, VulkanBuffer* buffer, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties)
{
    if (sizeInBytes == 0)
        return false;
    ...
    *buffer = VulkanBuffer();
    ...
    const int memoryTypeIndex = FindMemoryTypeIndex(physicalDeviceProperties, memoryRequirements, properties);
    if (memoryTypeIndex < 0)
    {
        ImmediateDestroyVulkanBuffer(*buffer);
        return false;
...
void NativeRenderVulkan::createVertexBuffer2(NativeMesh* mesh)
{
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    m->vertexBuffer;

    UnityVulkanBuffer bufferInfo;
    if (!m_UnityVulkan->AccessBuffer(mesh->vertexBuffer, 0, 0, kUnityVulkanResourceAccess_ObserveOnly, &bufferInfo))
        return;
    m->vertexBuffer = bufferInfo.buffer;
}
void NativeRenderVulkan::createIndexBuffer2(NativeMesh* mesh)
{
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    m->vertexBuffer;

    UnityVulkanBuffer bufferInfo;
    if (!m_UnityVulkan->AccessBuffer(mesh->indexBuffer, 0, 0, kUnityVulkanResourceAccess_ObserveOnly, &bufferInfo))
        return;
    m->indexBuffer = bufferInfo.buffer;
}

void NativeRenderVulkan::createIndexBuffer(NativeMesh* mesh)
{
    //VulkanMesh* m = (VulkanMesh*)mesh->native;
    //VkDeviceSize bufferSize = sizeof(int16_t) * mesh->indexCount;
    ...
    //void* data;
    //vkMapMemory(m_Instance.device, stagingBufferMemory, 0, bufferSize, 0, &data);
    //memcpy(data, mesh->indexBuffer, (size_t)bufferSize);
    //vkUnmapMemory(m_Instance.device, stagingBufferMemory);
    ...
    VulkanMesh* m = (VulkanMesh*)mesh->native;
    VkDeviceSize bufferSize = 4 * 2 * mesh->indexCount;
...
VkPipeline NativeRenderVulkan::CreateTrianglePipeline(VkDevice device, VkPipelineLayout pipelineLayout, VkRenderPass renderPass, VkPipelineCache pipelineCache)
{
    ...
    char* pathvar;
    pathvar = getenv("PATH");
    std::string env = pathvar;

    auto vertShaderCode = readFile("D:\\Project\\work\\client\\tis_client\\client\\slg_client\\Assets\\NativeShader\\vert.spv");
    auto fragShaderCode = readFile("D:\\Project\\work\\client\\tis_client\\client\\slg_client\\Assets\\NativeShader\\frag.spv");
    ...
    if (success)
    {
        ...
        moduleCreateInfo.pCode = reinterpret_cast<const uint32_t*>(vertShaderCode.data());// Shader::vertexShaderSpirv;
    ...
    if (success)
    {
        VkShaderModuleCreateInfo moduleCreateInfo = {};
        moduleCreateInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        moduleCreateInfo.codeSize = fragShaderCode.size();
        moduleCreateInfo.pCode = reinterpret_cast<const uint32_t*>(fragShaderCode.data());
    ...
    if (success)
    {
        ...
        //vertexInputAttributes[2].offset = 20;

        //vertexInputAttributes[3].binding = 0;
        //vertexInputAttributes[3].location = 3;
        //vertexInputAttributes[3].format = VK_FORMAT_R32G32B32A32_UINT;
...
void NativeRenderVulkan::DrawMesh(NativeMesh* mesh, const float* worldMatrix)
{
    UnityVulkanRecordingState recordingState;
    if (!m_UnityVulkan->CommandRecordingState(&recordingState, kUnityVulkanGraphicsQueueAccess_DontCare))
        return;
    ...
    if (m_TrianglePipeline != VK_NULL_HANDLE && m_TrianglePipelineLayout != VK_NULL_HANDLE)
    {
        ...
        if (mesh->native == NULL)
        {
            createNativeMesh(mesh);
        }

        if (!mesh->native)
            return;

        VulkanMesh* vmesh = (VulkanMesh*)mesh->native;
        ...
        VkBuffer vertexBuffers[] = { vmesh->vertexBuffer };

        VkDeviceSize offsets[] = { 0 };
        vkCmdBindVertexBuffers(recordingState.commandBuffer, 0, 1, vertexBuffers, offsets);

        vkCmdBindIndexBuffer(recordingState.commandBuffer, vmesh->indexBuffer, 0, VK_INDEX_TYPE_UINT16);
        ...
        vkCmdDrawIndexed(recordingState.commandBuffer, mesh->indexCount, 1, 0, 0, 0);
