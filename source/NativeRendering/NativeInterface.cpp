...
extern "C" int UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API SetNativeMeshFromUnity(void* vertexBufferHandle, int vertexCount, void* indexBufferHandle, int indexCount)
{
	NativeMesh* mesh = new NativeMesh{
	};

	mesh->vertexBuffer = vertexBufferHandle;
	mesh->vertexCount = vertexCount;
	mesh->indexBuffer = indexBufferHandle;
...
extern "C" int UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API SetNativeMeshFromUnity2(void* vertexBufferHandle, int vertexCount, void* indexBufferHandle, int indexCount)
{
	NativeMesh* mesh = new NativeMesh{
	};
...
extern "C" void	UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API UnityPluginLoad(IUnityInterfaces * unityInterfaces)
{
	...
	NM->unityInterfaces = unityInterfaces;
	s_Graphics = s_UnityInterfaces->Get<IUnityGraphics>();
	NM->graphics = s_Graphics;
...
static void UNITY_INTERFACE_API OnGraphicsDeviceEvent(UnityGfxDeviceEventType eventType)
{
	...
	if (eventType == kUnityGfxDeviceEventInitialize)
	{
		...
		NM->s_DeviceType = s_DeviceType;
		s_CurrentAPI = CreateRenderAPI(s_DeviceType);
		NM->renderApi = s_CurrentAPI;
	}
...
static void DrawMeshs()
{
	...
	s_CurrentAPI->DrawMesh(NM->meshs[0], worldMatrix);
}
